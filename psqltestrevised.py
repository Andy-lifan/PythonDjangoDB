...
def main():
    try:
        ...
        cnnx = psycopg2.connect(user='*****', password='*****', host='localhost',database='dbtest1')
        cursor = cnnx.cursor ()
        
        #create a table
        sql = " create table if not exists tb_test (employeeid int, lastname varchar(50), firstname varchar(50));"
        cursor.execute (sql)
